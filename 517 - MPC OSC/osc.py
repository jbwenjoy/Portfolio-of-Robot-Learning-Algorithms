...
class OperationalSpaceWalkingController(LeafSystem):
    ...
    def SetupAndSolveQP(self,  context: Context) -> Tuple[np.ndarray, MathematicalProgram]:
        ...
        for traj_name in self.trajs:
            ...
            Q_i = 2 * J_i.T @ W_i @ J_i
            b_i = ((JdotV_i.T @ (W_i + W_i.T) @ J_i - yddot_cmd_i.T @ (W_i + W_i.T) @ J_i)).T
            c_i = JdotV_i.T @ W_i @ JdotV_i - yddot_cmd_i.T @ (W_i + W_i.T) @ JdotV_i + yddot_cmd_i.T @ W_i @ yddot_cmd_i
            prog.AddQuadraticCost(0.5 * vdot.T @ Q_i @ vdot + b_i.T @ vdot + c_i)

        # TODO: Add Quadratic Cost on vdot using self.gains.w_vdot
        prog.AddQuadraticCost(0.5 * self.gains.w_vdot * vdot.T @ vdot)
        ...
        # print("Dyn con")
        # dyn_expr = M @ vdot + Cv + G - B @ u - J_c.T @ lambda_c
        for i in range(self.plant.num_velocities()):
            prog.AddLinearEqualityConstraint((M @ vdot + Cv + G - B @ u - J_c.T @ lambda_c)[i] == 0)

        # TODO: Add Contact Constraint
        # print("Contact con")
        for i in range(3):
            prog.AddLinearEqualityConstraint((J_c @ vdot + J_c_dot_v)[i] == 0)
        # prog.AddLinearEqualityConstraint(J_c @ vdot + J_c_dot_v, np.zeros(7))

        # TODO: Add Friction Cone Constraint assuming mu = 1
        # print("Fri con")
        mu = 1
        A_fl = np.array([[1, 0, -mu], [-1, 0, -mu]])
        for i in range(2):
            prog.AddLinearConstraint((A_fl @ lambda_c)[i] <= 0)
